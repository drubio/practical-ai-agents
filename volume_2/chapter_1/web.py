...
from shared.web import (
    ...
    run_uvicorn_app,
    supports_history,
    supports_reset_memory,
    to_sse_line,
)
from utils import get_provider_options, get_chapter_logger, normalize_agent_api_payload
...
def _resolve_request_manager(manager, requested_model: Optional[str]):
    auto_selection_values = {None, "", getattr(manager, "auto_provider_option_name", None), getattr(manager, "active_model_identifier", None), getattr(manager, "model", None)}
...
def _build_query_response(result: dict, request: QueryRequest, manager, *, stream_override: Optional[bool] = None) -> dict:
    ...
    response_payload = {"success": True, "framework": getattr(manager, "framework", "agent"), "provider": result.get("provider"), "model": result.get("model"), "response": normalized["response"], "raw_answer": normalized.get("raw_answer"), "prompt": result.get("prompt", request.template.format(topic=request.topic)), "parameters": {"temperature": request.temperature, "max_tokens": request.max_tokens, "template": request.template}, "session_id": result.get("session_id", effective_session_id)}
...
def create_web_api(manager, enable_streaming: bool = False) -> FastAPI:
    app = FastAPI(title="Volume 2 Agent API", version="1.0.0", description="HTTP bootstrap for chapter agent workflows")
    app.add_middleware(CORSMiddleware, allow_origins=["*"], allow_credentials=True, allow_methods=["*"], allow_headers=["*"])

    @app.get('/')
    async def status():
        logger.debug('Status endpoint called')
        return {"framework": getattr(manager, "framework", "agent"), "available_providers": [getattr(manager, "provider", "unknown")], "total_available": 1, "initialization_status": {getattr(manager, "provider", "unknown"): "Ready"}, "status": "healthy"}

    @app.get('/providers')
    async def providers():
        options = _manager_provider_options(manager)
        return {"framework": getattr(manager, "framework", "agent"), "providers": options, "count": len(options)}

    @app.get('/capabilities')
    async def capabilities():
        return {"framework": getattr(manager, "framework", "agent"), "streaming": True, "memory": supports_history(manager) and supports_reset_memory(manager), "memory_retrieval": False, "coagent": False}
    ...
    async def _stream_query(payload: QueryRequest):
        try:
            ...
            streamed_text = ''
            if stream_enabled:
                async for chunk in iter_text_chunks(normalize_response_text(raw_response)):
                    if chunk:
                        streamed_text += chunk
            ...
            done_payload = {"type": "done", "provider": result.get('provider'), "model": result.get('model'), "active_model_identifier": active_identifier, "response": normalized['response'] if isinstance(normalized.get('response'), dict) else None, "raw_answer": normalized.get('raw_answer'), "session_id": result.get('session_id', payload.sessionId or payload.session_id or 'default')}
            if 'tool_calls' in normalized:
                done_payload['tool_calls'] = normalized['tool_calls']
            yield to_sse_line(done_payload)
        except Exception as exc:
            logger.exception('/query-stream raised exception')
    ...
    @app.post('/query-all')
    async def query_all(_: QueryAllRequest):
    ...
    async def history(provider: str = 'openai', session_id: str = 'default'):
        if not supports_history(manager):
    ...
    async def reset_memory(request: Optional[ResetMemoryRequest] = Body(None), provider: Optional[str] = None, session_id: Optional[str] = None):
        if not supports_reset_memory(manager):
            raise HTTPException(status_code=400, detail='Session memory not supported by this manager')
        ...
        return manager.reset_memory(body_provider if body_provider is not None else provider, body_session_id if body_session_id is not None else session_id)

    return app


def run_web_server(manager, host: str = '0.0.0.0', port: int = 8000, enable_streaming: bool = False):
    run_uvicorn_app(create_web_api(manager, enable_streaming), getattr(manager, 'framework', 'agent'), host=host, port=port)
